...
class Encoder(nn.Module):
    ...
    def init_hidden(self, batch_size):
        ...
        hidden = torch.zeros(2*self.n_layers, batch_size, self.hidden_dim).to(self.DEVICE)
...
class Attention(nn.Module):
    ...
    def init_hidden(self, batch_size):
        ...
        s_inner_hidden = torch.zeros(self.n_layers, batch_size, self.rnn_hidden_size).to(self.DEVICE)
        v_output_hidden= torch.zeros(1, batch_size, self.output_size).to(self.DEVICE)
...
class Seq2Graph(nn.Module):
    ...
    def forward(self, model_input):
        ...
        for i in range(self.num_of_timeseries):
            ...
            dp_list.append(dualpurpose_output.transpose(0,2))
